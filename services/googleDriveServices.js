...
const channelId = `arkive-drive-webhook-${randomUUID()}`;
...
export const registerDriveWebhook = async () => {
  try {
    ...
    const startToken = (await drive.changes.getStartPageToken()).data
      .startPageToken;

    const res = await drive.changes.watch({
      pageToken: startToken,
      requestBody: {
        id: channelId, // must be unique every time you register
        type: "web_hook",
